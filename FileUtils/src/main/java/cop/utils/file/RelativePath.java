...
public final class RelativePath {
	 ...
	 *
	 * @param base base path, should be a directory, not a file, or it doesn't
	 *             make sense
	 * @param file file to generate path for
	 * @return path from home to f as a string
	 */
	public static File getRelativePath(File base, File file) {
		return new File(matchPathLists(getPathList(base), getPathList(file)));
	}
	 ...
	 * 'f' with respect to 'r'
	 *
	 * @param base home path
	 * @param f    path of file
	 */
	private static String matchPathLists(List<String> base, List<String> f) {
		int i;
		int j;
		String s;
		...
		while ((i >= 0) && (j >= 0) && (base.get(i).equals(f.get(j)))) {
			i--;
			j--;
		}
